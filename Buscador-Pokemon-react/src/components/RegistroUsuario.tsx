...
export const RegistroUsuario: React.FC = () => {
  ...
  const [CiudadDeDomicilio, setCiudadDeDomicilio] = useState("");
  ...
  const eventoSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const nuevo: Usuario = {
      ...
      País_de_domicilio: PaísDeDomicilio,
      Ciudad_de_domicilio: CiudadDeDomicilio,
      fechaNacimiento,
      telefono,
      correo,
      datosPersonales,
  ...
  return (
    <div>
      ...
      <div>
        <form onSubmit={eventoSubmit} id="FormularioRegistro" action="#">
          ...
          <div className="form-group">
            ...
            <select
              id="pais"
              value={PaísDeDomicilio}
              onChange={(e) => setPaisDeDomicilio(e.target.value)}
              name="pais"
              required
            >
          ...
          <div className="form-group">
            ...
            <select
              id="ciudad"
              value={CiudadDeDomicilio}
              onChange={(e) => setCiudadDeDomicilio(e.target.value)}
              name="ciudad"
              required
            >
      ...
      {entrenadores.length > 0 && (
        <div>
          <h3 className="cambio-entrenador">Cambiar Entrenador</h3>
          <div>
            {entrenadores.map((user) => (
              <button
                key={user.id} type="button" onClick={() => seleccionarEntrenador(user)}
                style={{
                  backgroundColor: entrenadorActivo?.id === user.id ? "#e71616f6" : "#e0e0e0",
